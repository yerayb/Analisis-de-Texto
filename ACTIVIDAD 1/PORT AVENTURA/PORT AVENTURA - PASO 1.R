...
datos <- read.csv("PA-OP.csv") #llamamos al dataset creado en el paso 0
...
port_op = tm_map(corpus, PlainTextDocument)

#Creamos una nube de palabras
wordcloud(port_op , max.words = 30, random.order = F, colors = brewer.pal(name = "Dark2", n = 8))
